...
| ML-NDT | 超声 PAUT B-scan（minibatch 容器） | 316L 奥氏体管道焊缝 | 1 | 3（真实热疲劳裂纹） | 201 容器 / 20,010 张 eFlaw 增强 B-scan（12,128 缺陷 / 7,882 干净） | LGPL-3.0 | 174 MB | ✅ 免登录 | **P1**（预训练素材） |
...
| nominal sample | **201 个 minibatch 容器 = 20,010 张 B-scan**（train 199 容器 + val 2 容器；200×100 + 1×10 张 eFlaw 增强图，12,128 缺陷 / 7,882 干净；非三维体积采集） | 已确认 |
