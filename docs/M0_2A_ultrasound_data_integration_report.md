...
| ML-NDT | **201**（minibatch 容器，全部 201 个已落地；200 容器×100 张 + 1 容器×10 张 = 20,010 张） | 1（316L 管道焊缝） | 3 真实裂纹（1.6/4.0/8.6 mm）+ eFlaw 虚拟事件 | 实测 + 仿真增强 | (100, 256, 256) uint16 = **100 张增强 B-scan 的 minibatch 容器**（非三维体积采集） | 20,010 张 B-scan（12,128 缺陷 / 7,882 干净；单试件，模态完全匹配 PAUT；有效独立单元 ≈ 3 真实裂纹） |
...
> 三个数据集已**全部完整落地**（ML-NDT 201 个 minibatch 容器 = 20,010 张 B-scan（12,128 缺陷 / 7,882 干净）、
...
|---|---|---|---|---|---|
| B1 | 帧级 SSL 预训练 | ML-NDT 20,010 张帧（12,128 缺陷） | MAE（掩码重建） | PAUT LOOCV（非PP4 逐折） | 模态最匹配，能否降 feature shift |
