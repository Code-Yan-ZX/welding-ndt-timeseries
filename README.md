...
**主线结论**：PAUT 跨试件泛化困难（裸 SSF 非PP4 0.538）；域内 SSL 预训练（0.572）与通用多模态 LLM 视觉先验（0.593）两路有效，但注入物理/文本条件或微调反而退化--VLM 瓶颈是感知而非推理。**口径修正（P4a）**：P2 的 0.593（pooled）与 P1 的 0.572（逐折均值）不可比；统一口径下 SSL ≥ VLM。**天花板定位（P4a/P4b/P5）**：+20% test 折标签无效（0.583）、融合/微调/TTA/形态头/掩码目标/合成注入全部多 seed 证伪 → 瓶颈是 5 试件数据结构（缺陷率 0.5%–76% 与试件身份强耦合）下的表征级上限 ~0.58–0.60（逐折均值）。**P5 关键诊断**：物理保真合成注入的标签也无效（学到的"高斯峰"特征不能迁移到真实缺陷），证明瓶颈是"真实缺陷形态空间"本身的跨试件变异，不是合成可行性。**翻盘路径（资源型）**：新试件数据解耦"缺陷存在"与"试件缺陷率"、或 CIVA 级合成教"缺陷回波物理"（H5 已证同分布加标签无效；P5 已证物理注入也无效——需 CIVA 级而非简化高斯）。汇总对比表见 [`experiments/results/paut_loocv_table.md`](experiments/results/paut_loocv_table.md)。
