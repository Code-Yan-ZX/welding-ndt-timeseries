...
| P2 | 多模态 LLM（Qwen3.6-27B）零样本 B-scan QA + LoRA | 0.593（pooled） | **被证伪/修正**：0.593 是 pooled 口径，与 SSL 逐折均值 0.572 不可比；统一口径后 **SSL ≥ VLM**（pooled 0.607 vs 0.600；逐折 0.572 vs 0.536），**VLM 从未超过 SSL、非最优** | [`PAUT_P2`](reports/PAUT_P2_多模态LLM实验报告.md) |
| P3 | 物理条件化 + 推理 CoT + LoRA 5 折 | 0.512 / 0.508 / 0.510（physics/physics_cot 为 pooled，LoRA 为逐折均值） | **负面**：物理/CoT/微调均低于 bare(0.600, pooled)，瓶颈在感知而非推理 | [`PAUT_P3`](reports/PAUT_P3_物理条件化多模态LLM实验报告.md) |
