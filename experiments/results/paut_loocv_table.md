...
## P5b 跨试件监督对比学习 SupCon (2026-08-12, 1 seed per-fold 严格) ❌ 假象撤回

**⚠️ 关键诚实性修正**: 首版 P5b pretrain 用全 5 试件 (含 test) 标签, 0.985 是**信息泄露假象** (编码器已"见过" test 折的 defect 模式)。严格 per-fold pretrain (cold-start, pretrain 完全不接触 test 试件) 真实结果:
...
| P5b 原版 (全 5 试件 pretrain) | ~~0.985±0.012~~ | ~~0.989±0.007~~ | - | - | - | - | **信息泄露假象 (撤稿)** |
| **P5b per-fold 严格 (cold-start)** | **0.485** | **0.454** | 0.453 | 0.507 | 0.493 | 0.487 | **比 baseline 低 0.094, 比 P5a 还低** |

**P5b 教训**:
- **val-test gap 0.51 (P5b 严格)** vs baseline 0.27 → 严重过拟合到训练试件
- **0.985 是 val 0.999 (训练试件内) + test 0.999 (test 试件已"见过")** 的双重假象
- **信息泄露**: 任何 pretrain 阶段用了"全部试件数据"的方案, 都不能作为 LOOCV 的真"跨域"评估
- **方法学教训**: 监督 SSL 评估必须 per-fold 严格 pretrain; val-test gap 是真假单一最可靠指示器
