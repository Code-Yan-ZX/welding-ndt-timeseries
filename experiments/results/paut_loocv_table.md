...
> **统一口径 (2026-08-13)**: 本表与 README 汇总表所有 SSL 编码器下游数字均为**规范头协议**
> (冻结 SSL 编码器 + 分类头 lr=1e-3/epochs=80, 即 P1 SSL 同超参; P4b/P5/P5b 已按此重跑确认)。
> 弱头协议 (lr=5e-4/epochs=40) 已弃用, 不再列入。VLM 行数字为 pooled/逐折均值混合口径,
> 仅作图像路线参考, **非最优** (统一口径下 SSL ≥ VLM)。
>
> **指标规则 (主/参考分离)**: **主指标 = 非PP4 逐折均值** (每折独立评估后平均, 不被少数试件主导)。
> **pooled (跨试件拼分数) 仅作参考, 不作主指标** — 试件缺陷率悬殊 (0.5%-76%) 时 pooled 被
> **试件级偏差**虚高 (高缺陷率试件整体高分 + 低缺陷率试件整体低分 = 白送排序): P4a 已证伪融合
> pooled 0.668 为试件级伪增益; VLM 0.600 的 pooled−逐折差 0.064 全项目最大 (逐折仅 0.536),
> SSL 仅 0.035。pooled 与逐折均值不可混用, 结论以逐折均值为准。
...
下游: 冻结编码器 + 二分类头 LOOCV (规范头协议 lr=1e-3/80ep), 3 seed (42/43/44), 逐折均值。

| 预训练掩码目标 | 非PP4 逐折均值 | nonPP4 pooled | PP3 | PP5 | PP6 | PP7 | 结论 |
|---|---|---|---|---|---|---|---|
| beam (P1 原版) | **0.579±0.007** | 0.616±0.016 | 0.493 | 0.539 | 0.566 | 0.685 | 最强 |
| both (beam+depth) | 0.566±0.000 | 0.642±0.009 | 0.464 | 0.564 | 0.597 | 0.641 | ≈beam, 略低 |
| depth (深度块) | 0.511±0.007 | 0.608±0.005 | 0.477 | 0.495 | 0.496 | 0.576 | **显著负面** |
...
改造 SSL 任务本身: 在 raw A-scan 上注入物理保真的合成缺陷 (2D 高斯峰, A∈800-3000 raw, σ_b∈0.8-2.0, σ_t∈2.0-6.0), 多任务训练 (w_recon=0.1 MAE 重建 + w_inj=1.0 注入检测 + w_loc=1.0 7×16 网格定位)。其他与 P1 完全同协议 (同数据 11980/同 40ep)。下游: 冻结编码器 + 二分类头 LOOCV (规范头协议 lr=1e-3/80ep), 3 seed (42/43/44)。
...
| P5 s42 | 0.9971 | 0.540 | 0.578 | 0.492 | 0.507 | 0.514 | 0.648 | 负面 |
| P5 s43 | 0.9987 | 0.545 | 0.581 | 0.497 | 0.491 | 0.602 | 0.590 | 负面 |
| P5 s44 | 0.9983 | 0.549 | 0.580 | 0.520 | 0.484 | 0.555 | 0.637 | 负面 |
| **P5 mean±std** | **0.9980±0.0007** | **0.545±0.003** | **0.580±0.001** | 0.503±0.012 | 0.494±0.010 | 0.557±0.036 | 0.625±0.025 | **比 baseline 低 0.034** |
...
## P5b 跨试件监督对比学习 SupCon (2026-08-12, 1 seed per-fold 严格)

**关键**: per-fold 严格 pretrain (cold-start, pretrain 只用 4 试件标签, 完全不接触 test 试件) — 这是任何监督 SSL 评估的真"跨域"协议。
...
| P5a 缺陷注入 SSL | 0.545±0.003 | 0.580±0.001 | 0.503 | 0.494 | 0.557 | 0.625 | 负面 (合成 ≠ 真实) |
| **P5b SupCon per-fold 严格** | **0.487** | **0.478** | 0.451 | 0.507 | 0.504 | 0.488 | **负面, 比 baseline 低 0.092, 比 P5a 还低** |

**P5b 关键诊断**:
- val-test gap 0.51 (P5b) vs baseline 0.27 → 严重过拟合到训练试件
- 4 试件 ~2400 位置, SupCon 把 4 试件的"共有判别模式"学得过死, 无法泛化到第 5 试件
- 监督 SSL 评估必须 per-fold 严格 pretrain; val-test gap 是真假单一最可靠指示器
