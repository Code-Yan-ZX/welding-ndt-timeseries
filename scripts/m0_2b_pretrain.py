...
Usage:
  python scripts/m0_2b_pretrain.py external --steps 10000 --seed 42
  python scripts/m0_2b_pretrain.py target --fold PP3 --steps 10000 --seed 42
  python scripts/m0_2b_pretrain.py target --fold PP3 --steps 2000 --seed 42 \\
      --init experiments/runs/m0_2b/pretrain/external_s42_s8000.pt
...
class PretrainOpts:
    ...
    def from_args(cls, args) -> "PretrainOpts":
        return cls(
            cmd=args.cmd, seed=args.seed, steps=args.steps,
...
def external_ckpt_path(seed: int, steps: int) -> Path:
    return PRETRAIN_DIR / f"external_s{seed}_s{steps}.pt"


def target_ckpt_path(seed: int, fold: str, steps: int, init_tag: str | None = None) -> Path:
    suffix = f"_init-{init_tag}" if init_tag else ""
    return PRETRAIN_DIR / f"target_s{seed}_fold{fold}_s{steps}{suffix}.pt"
...
def run_pretrain_loop(model, total_steps, batch_size, base_lr, weight_decay,
                      warmup_steps, grad_clip, seed, device,
                      batch_fn) -> dict:
    """通用 MAE 预训练循环。``batch_fn(global_step) -> torch (B,1,H,W)``。

    返回 ``{loss_log, n_batches}``。
    """
    set_seed(seed)
...
def pretrain_external(opts: PretrainOpts, cfg, device) -> Path:
    ...
    steps = opts.steps
    p = cfg.pretrain
    out_path = external_ckpt_path(seed, steps)
    ...
    # NDT_ML_Flaw float16 局部窗口缓存（profile 证实流式单条带读取反复整批
    # 解压 ~11s/次必然卡 GPU；缓存每批只解压一次，可重建）
    cache = NDTWindowCache(seed, n_steps=steps * batch_size, batch_size=batch_size,
    ...
    model = build_model(cfg).to(device)
    total_steps = steps
    print(f"[external] ML-NDT + NDT_ML_Flaw 均衡 SSL | seed={seed} steps={steps} "
          f"batch={batch_size} ({total_steps * batch_size} samples)")

    def batch_fn(k: int):
        source = external_dataset_for_batch(k)
        x_np = build_external_batch(
            source, [k * batch_size + j for j in range(batch_size)], seed,
    ...
    run_info = run_pretrain_loop(
        ...
        int(p.warmup_steps), float(p.grad_clip), seed, device, batch_fn)

    meta = {
        "exp": "external_ssl", "seed": seed, "steps": steps,
...
def pretrain_target(opts: PretrainOpts, cfg, device) -> Path:
    ...
    out_path = target_ckpt_path(seed, opts.fold, steps, init_tag=init_tag)
    if out_path.exists() and not opts.force:
    ...
    print(f"[target] PENELOPE SSL | fold(test={opts.fold}) train_coupons="
          f"{train_coupons} n={n} | seed={seed} steps={steps} batch={batch_size} "
          f"({steps * batch_size} samples, {steps * batch_size / n:.1f} passes)")

    def batch_fn(k: int):
        # 确定性随机抽样（seeded global RNG）
        idx = torch.randint(0, n, (batch_size,))
        return torch.from_numpy(X[idx.numpy()]).to(device, non_blocking=True)
    ...
    meta = {
        "exp": "target_ssl", "seed": seed, "fold_test": opts.fold,
...
def main():
    ...
    print(f"M0-2B pretrain[{args.cmd}] seed={args.seed} steps={args.steps} "
