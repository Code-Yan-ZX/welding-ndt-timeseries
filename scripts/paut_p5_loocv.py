#!/usr/bin/env python3
"""P5 LOOCV 评估 — 冻结 P5 编码器 + 二分类头, 5 折 LOOCV (同 P4a baseline 协议)。

协议: 与 P4a baseline 一致
  - 折: PP3/PP4/PP5/PP6/PP7 轮流 test
  - 训练: 其余 4 折 85/15 分层切 train/val, val-AUC 早停 (patience 20)
  - 优化: AdamW, lr=5e-4, wd=1e-4, batch=128
  - 加权采样 (WeightedRandomSampler)
  - epochs=40
  - 主指标: 非PP4 逐折均值 + 非PP4 pooled
  - 多 seed: 42, 43, 44 (P4a 多 seed 教训: 单 seed 不算)

Usage:
  python scripts/paut_p5_loocv.py --ckpt experiments/runs/ssl_p5_inject_s42/encoder.pt --seed 42
"""
...
def run(args, device):
    ...
    for tc in COUPONS:
        ...
        # P5 encoder saved with "conv." and "proj." stripped.
        # conv has params at indices 0,1,4,5,8,9 (Conv2d+BN).
        # proj has params at index 2 (Linear, the only one with weight).
        ...
        print(f"  [P5] test={tc}: AUC={auc:.3f} val={fit['best_val_auc']:.3f} "
    ...
    summary = {"exp": "p5_inject", "seed": args.seed, "ckpt": str(args.ckpt),
               "nonPP4_fold_mean": float(np.mean(np4)),
               "nonPP4_pooled": pooled_auc,
               "folds": [{k: v for k, v in f.items() if k != "scores"} for f in folds]}
    print(f"[P5 seed={args.seed}] nonPP4 逐折均值={np.mean(np4):.3f} pooled={pooled_auc:.3f}")
    out_tag = Path(args.ckpt).parent.name
    with open(RES / f"paut_p5_{out_tag}_s{args.seed}_full.json", "w") as fh:
...
def main():
    ...
    print(f"device={device} ckpt={args.ckpt} seed={args.seed}")
    run(args, device)
